...
const BlockNews = React.lazy(() =>
    import ('../../components/BlockNews/BlockNews'));
const LoginPage = React.lazy(() =>
    import ('../../components/BlockLogin/LoginPage'));
...
const App = () => {
        return ( <
                ...
                BlockNavigation / >
                <
                div className = 'app-wrapper-content' > { /* Страница Сообщения */ } <
                Route path = '/dialogs'
                render = {
                    () => < BlockDialogsContainer / > }
                /> { /* Страница Посты */ } <
                Route path = '/contents'
                render = {
                    () => < BlockContent / > }
                ...
                render = {
                    () => {
                        return <Suspense fallback = { < Preloader / > } > < BlockNews / > < /Suspense> }} / > { /* Страница Список Пользователей */ } <
                            Route path = '/users'
                        render = {
                            () => < BlockUsersContainer / > }
